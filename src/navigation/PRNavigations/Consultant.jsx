...
const ConsultantHistory = () => {
  ...
  const handlePress = (id) => {
    navigation.navigate("ConsultantLogs", { id });
  };
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${apiKey}/purchase-request?page=${page}&per_page=15&tab=3&branch=${selectedBranch}&department=${selectedDepartment}&category=${selectedCategory}&item_group=${selectedItemGroup}&requested_date=${selectedStartDate}&required_date=${selectedEndDate}`,
          {
            headers: {
    ...
    return (
      <TouchableOpacity onPress={() => handlePress(item.id)}>
        <PRCard
          ...
          dateApproved={item?.pr_Branch_Level2_ApprovedDate}
